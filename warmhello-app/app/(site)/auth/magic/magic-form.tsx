...
export function MagicForm({ token, destination }: MagicFormProps) {
  ...
  useEffect(() => {
    ...
    async function apply() {
      if (!token) {
        setStatus("invalid");
        setMessage(
          "This log-in link is missing its security token. Please request a new link.",
        );
        return;
      }
      ...
      try {
        ...
        if (data.ok && next === "ok") {
          ...
          const target = data.redirect ?? destination ?? "/dashboard";
        ...
        if (!response.ok || response.status >= 500 || next === "error") {
          ...
          setMessage(
            data.message ??
              "Something went wrong while verifying your link. Please try again in a moment.",
          );
          return;
        }
        setStatus("invalid");
        ...
        <article className="card auth-panel auth-panel-active">
          {status === "loading" ? (
            ...
            <>
              ...
              <Link href={destination as unknown as any} className="button primary auth-submit">
                Go to Dashboard
              </Link>
            </>
          ) : (
